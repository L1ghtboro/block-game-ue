...
// Permutation table
int PerlinNoiseOverride::Permutation[256] = {
...
// Repeat the permutation table twice
int PerlinNoiseOverride::P[512];

float PerlinNoiseOverride::Fade(float t) {
    // Fade function as defined by Ken Perlin
    return t * t * t * (t * (t * 6 - 15) + 10);
}

float PerlinNoiseOverride::Lerp(float t, float a, float b) {
    // Linear interpolation
    return a + t * (b - a);
}

float PerlinNoiseOverride::Grad(int hash, float x, float y, float z) {
    // Convert the lower 4 bits of the hash code into 12 gradient directions
    int h = hash & 15;
    float u = h < 8 ? x : y;
...
float PerlinNoiseOverride::Noise(float x, float y, float z) {
    ...
    return (res + 1.0f) / 2.0f; // Scale to range [0, 1]
}
